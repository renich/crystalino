...
class Crystalino::Project
  ...
  # or closed.
  @source_index : Crystalino::Lightweight::Index?

  def source_index : Crystalino::Lightweight::Index?
    @source_index ||= build_source_index
  end

  def source_index=(index : Crystalino::Lightweight::Index?)
    @source_index = index
  end
  ...
  getter flags : Array(String) do
    (shard_yaml.dig?("crystalino", "flags").try(&.as_a.map(&.as_s)) || [] of String).tap do |flags|
  ...
  def self.find_in_workspace_root(workspace_root_uri : URI) : Array(Project)
    ...
    # First, check for a Crystalino project file.
    begin
      ...
      projects = shards_yaml.dig?("crystalino", "projects").try do |pjs|
  ...
  private def build_source_index : Crystalino::Lightweight::Index?
    files = [] of String
    ...
    indexes = [] of Crystalino::Lightweight::Index
    files.each do |file|
      next unless File.file?(file)
      if index = Crystalino::Lightweight::Index.from_source(File.read(file), file)
        indexes << index
      end
    end
    ...
    Crystalino::Lightweight::Index.merge(indexes)
  end
