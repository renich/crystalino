module Crystalino::Analysis
  class DocumentSymbolsVisitor < Crystal::Visitor
    ...
    private def create_symbol_from_node(node : Crystal::ASTNode, kind : LSP::SymbolKind, detail : String? = nil)
      range = AstResolver.lsp_range_from_node(@parent_macro_call || node)
    ...
    def visit(node : Crystal::Def)
      symbol = create_symbol_from_node(node, :function, detail: DefFormatter.format_def(node))
      append_symbol(symbol)
      false
    end

    def visit(node : Crystal::Macro)
      symbol = create_symbol_from_node(node, :method, detail: DefFormatter.format_def(node))
      append_symbol(symbol)
      false
    end
