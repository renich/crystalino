...
module Crystalino::Lightweight
  class Completion
    def self.complete(source : String, line_number : Int32, context : Crystalino::CompletionContext, query : Query) : Array(LSP::CompletionItem)?
      complete_and_reason(source, line_number, context, query)[0]
    end

    def self.diagnose(source : String, line_number : Int32, context : Crystalino::CompletionContext, query : Query) : String
    ...
    def self.complete_and_reason(source : String, line_number : Int32, context : Crystalino::CompletionContext, query : Query) : {Array(LSP::CompletionItem)?, String}
      new(source, line_number, context, query).complete_or_reason
    end

    def initialize(@source : String, @line_number : Int32, @context : Crystalino::CompletionContext, @query : Query)
    end
    ...
    private def complete_methods : Array(LSP::CompletionItem)?
      ...
      type_names.each do |type_name|
        ...
        if Crystalino::Lightweight::Query.subtype_fallback_type?(type_name)
