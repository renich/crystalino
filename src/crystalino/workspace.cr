...
class Crystalino::Workspace
  ...
  @result_cache : Crystalino::ResultCache = Crystalino::ResultCache.new
  ...
  def recalculate_dependencies(server, project)
    ...
    Analysis.compile(server, target, lib_path: lib_path, ignore_diagnostics: true, wants_doc: true, top_level: true, compiler_flags: project.flags).try { |result|
      ...
      summary, index = Analysis.run_dedicated do
        {
          Crystalino::Lightweight::Summary.from_result(result),
  ...
  private def lightweight_query_for(document : TextDocument) : Crystalino::Lightweight::Query?
  ...
  private def build_lightweight_query_from_project(document : TextDocument) : Crystalino::Lightweight::Query?
    ...
    if document.dirty? || !project.dependencies.includes?(document.uri.decoded_path)
      source_index = Crystalino::Lightweight::Index.from_source(fix_source(document.contents), document.uri.decoded_path)
      Crystalino::Lightweight::Query.new(project_index, project.semantic_summary, secondary: Crystalino::Lightweight::PreludeIndex.get, overlay: source_index)
    else
      Crystalino::Lightweight::Query.new(project_index, project.semantic_summary, secondary: Crystalino::Lightweight::PreludeIndex.get)
    end
  end

  private def build_lightweight_query_from_source(document : TextDocument) : Crystalino::Lightweight::Query?
    source_index = Crystalino::Lightweight::Index.from_source(fix_source(document.contents), document.uri.decoded_path)
    ...
    prelude = Crystalino::Lightweight::PreludeIndex.get

    if project_index
      if prelude
        Crystalino::Lightweight::Query.new(project_index, secondary: prelude, overlay: source_index)
      else
        Crystalino::Lightweight::Query.new(project_index, overlay: source_index)
      end
    elsif prelude
      Crystalino::Lightweight::Query.new(prelude, overlay: source_index)
    else
      Crystalino::Lightweight::Query.new(source_index)
    end
  end
  ...
  private def build_def_macro_hover(node, contents)
    contents << code_markdown(DefFormatter.format_def(node), language: "crystal")
  ...
  private def build_call_hover(node, contents)
    if definition = node.target_defs.try &.first?
      contents << code_markdown(DefFormatter.format_def(definition), language: "crystal")
  ...
  private def build_path_hover(node, nodes, contents)
    node_type = node.type? || Analysis::AstResolver.resolve_path(node, nodes)
    if node_type
    ...
    if node_type && node_type.responds_to?(:defs)
      Analysis.all_defs(node_type).each { |def_name, definition, owner_type, nesting|
        ...
        completion_items << LSP::CompletionItem.new(
          label: DefFormatter.format_def(definition, short: true),
          insert_text: def_name,
          kind: LSP::CompletionItemKind::Function,
          filter_text: def_name,
          detail: DefFormatter.format_def(definition),
      ...
      Analysis.all_macros(node.type).each { |macro_name, macro_def, owner_type, nesting|
        ...
        completion_items << LSP::CompletionItem.new(
          label: DefFormatter.format_def(macro_def, short: true),
          insert_text: macro_name,
          kind: LSP::CompletionItemKind::Method,
          filter_text: macro_name,
          detail: DefFormatter.format_def(macro_def),
  ...
  private def add_module_completions(node, nodes, range, trigger_character, result, completion_items)
    node_type = node.type?
    node_type ||= Analysis::AstResolver.resolve_path(node, nodes) if node.is_a? Crystal::Path
    ...
    if node_type.is_a? Crystal::MetaclassType
      ...
      Analysis.all_submodules(result, node_type).uniq(&.to_s).each { |type|
        ...
        completion_items << LSP::CompletionItem.new(
          ...
          kind: Lightweight::CompletionKind.map(type, default: LSP::CompletionItemKind::Module),
