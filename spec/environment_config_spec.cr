...
describe Crystalino::EnvironmentConfig do
  describe ".unquote_env_value" do
    it "passes unquoted values through" do
      Crystalino::EnvironmentConfig.unquote_env_value("/usr/lib/crystal").should eq("/usr/lib/crystal")
    end

    it "strips simple single-quoted values" do
      Crystalino::EnvironmentConfig.unquote_env_value("'-Dfoo'").should eq("-Dfoo")
    end

    it "decodes empty quoted values" do
      Crystalino::EnvironmentConfig.unquote_env_value("''").should eq("")
    end

    it "decodes single quotes escaped as \"'\"" do
      Crystalino::EnvironmentConfig.unquote_env_value(%q{'a'"'"'b'}).should eq("a'b")
    end
    ...
    it "leaves malformed quoting untouched" do
      Crystalino::EnvironmentConfig.unquote_env_value("'unterminated").should eq("'unterminated")
      Crystalino::EnvironmentConfig.unquote_env_value("unopened'").should eq("unopened'")
    end
  end
  ...
  describe ".parse_crystal_env_output" do
    it "keeps '=' characters inside env values" do
      ...
      result = Crystalino::EnvironmentConfig.parse_crystal_env_output(
